...
/*
	...
	#define PPS_LENGTH 112
	static uint8_t pps_table[PPS_LENGTH] = {
		0x11,0x00,0x00,0x89,0x30,0x80,0x05,0xa0,
		0x05,0xa0,0x00,0x20,0x02,0xd0,0x02,0xd0,
		0x02,0x00,0x02,0x68,0x00,0x20,0x03,0x87,
		0x00,0x0a,0x00,0x0c,0x03,0x19,0x02,0x63,
		...
		0x08,0x00,0x02,0x07,0x00,0x00,0x00,0x00,
		0x00,0x00,0x5a,0x00,0x61,0xe0,0x01,0xc8
	};
#endif //#if defined(SHARP_LS029B3SX01_ONE_120Hz)
*/
...
//OCM auto-flash check flag, default value: on
static uint8_t auto_flash_flag			= FLAG_VALUE_ON;
...
static void DPCD_setting(void){
	...
	#ifdef DEBUG_LEVEL_2
		TRACE0("DPCD_setting(void)\n");
	#endif
	
	// move DPCD setting to OCM, keep DSC setting
	#if defined(SHARP_LS029B3SX01_ONE_120Hz)
		i2c_write_byte(SLAVEID_DPCD, MAX_LINK_RATE, 0x14);	//DPCD 00001 RBR
		i2c_write_byte(SLAVEID_DPCD, MAX_LANE_COUNT, 0xc2);	//DPCD 00002 2 lane
	#endif
}
...
char mipi_panel_power_supply(uint8_t onoff){
	...
	/*
		...
		if(onoff == PANEL_POWER_SUPPLY_OFF){
			...
			CHICAGO_PANEL0_ENPORT_ON();
			CHICAGO_PANEL1_ENPORT_ON();
			#ifdef DEBUG_LEVEL_2
				TRACE1("\t%s\n", "Panel enport HIGH");
			#endif
	...
	*/
	...
	// #if defined(SHARP_LS029B3SX02_FOUR_90Hz) || defined(SHARP_LS029B3SX04_FOUR_90Hz) || defined(BOE_2880_1600_60Hz)
...
static char DCS_sleep_control(uint8_t onoff){
	...
	if(onoff == PANEL_SLEEP_IN){
		...
		for(uint8_t i = 0; i < MIPI_TOTAL_PORT; i++){
			dcs.mipi_port = dcs_port_seq[i];
			dcs.data_type = DATASHORT_DCS_WRITE_1;
			dcs.param1 = 0xFF;
			dcs.param2 = 0x10;
			dcs.pData = NULL;
			mipi_packet_send_short(&dcs);
...
static void chicago_set_display_delay_ms(void){
	...
	static void DSC_PPS_enable_and_DCS_init(void){
        ...
        #ifdef SHARP_LS029B3SX01_ONE_120Hz
		
		uint8_t dsc_long1[17] = {
			0xC1, 0x09, 0x20, 0x00, 0x20, 0x02, 0x00, 0x02,
			0x68, 0x03, 0x87, 0x00, 0x0A, 0x03, 0x19, 0x02,
			0x63
		};
		...
		for(uint8_t i = 0; i < MIPI_TOTAL_PORT; i++){
			short_dcs.mipi_port = dcs_port_seq[i];
			short_dcs.data_type = DATASHORT_DCS_WRITE_1;
			short_dcs.param1 = 0x7A;
			short_dcs.param2 = 0x02;
			short_dcs.pData = NULL;
			mipi_packet_send_short(&short_dcs);
		}
		...
		for(uint8_t i = 0; i < MIPI_TOTAL_PORT; i++){
			short_dcs.mipi_port = dcs_port_seq[i];
			short_dcs.data_type = DATASHORT_COMPRESSION_MODE;
			short_dcs.param1 = 0x01;
		...
		// ryan sometimes display doesn't come out of sleep the first time? 
		/* 
...
void chicago_main(void){
	...
	case STATE_NONE:
		...
		#ifdef DEBUG_ENABLE_COLOR_BARS
			auto_flash_flag = FLAG_VALUE_OFF;
			dp_cable = DP_CABLE_IN;
		#endif 
		//}
		
		// OCM FW flash check				
		if(auto_flash_flag == FLAG_VALUE_ON)
		{
			burn_hex_auto();
			
			// No need do auto-flash again
			auto_flash_flag = FLAG_VALUE_OFF;
		}
