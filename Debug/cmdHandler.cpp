	...
	else
	{
		if (sscanf((const char *)g_CmdLineBuf, "\\%15s", CommandName) == 1)
		{
			...
			else if (strcmp((const char *)CommandName, "readhex") == 0)
			{
				power_restart();
				//delay_ms(10);
				if (sscanf((const char *)g_CmdLineBuf, "\\%*s %x %lx", &Flash_Addr, &size_to_be_read) == 2)
				{
					command_flash_read(Flash_Addr, size_to_be_read);
				}
				else
				{
					TRACE("\tBad parameter! Usage:\n");
			...
			else if (strcmp((const char *)CommandName, "ocmversion") == 0)
			{
...
static void help(void)
{
    ...
    TRACE("\t\\paneloff \\stopocm \\startocm \\ocmversion \\readintr \n\n");	

	TRACE("\t\\fl_se \\fl_ce \\erase \\readhex \\burnhex\n");
}
...
static void man(void)
{
	...
	if (sscanf((const char *)g_CmdLineBuf, (const char *)"\\%*s %s", CommandName) == 1)
	{
		...
		{
			TRACE("\tCommand: fl_se\n");
			TRACE("\tFunction: Flash Sector Erase\n");
			TRACE("\tUsage: \\fl_SE <address>\n");
		}
		else if (strcmp((const char *)CommandName, "fl_ce") == 0)
		{
			TRACE("\tCommand: fl_ce\n");
			TRACE("\tFunction: Flash Chip Erase\n");
			TRACE("\tUsage: \\fl_CE\n");
		}
		else if (strcmp((const char *)CommandName, "erase") == 0)
		{
			TRACE("\tCommand: erase\n");
			TRACE("\tFunction: Erase main OCM flash block\n");
			TRACE("\tUsage: \\erase\n");
		}
		else if (strcmp((const char *)CommandName, "readhex") == 0)
		{
			TRACE("\tCommand: readhex\n");
			TRACE("\tFunction: read OCM hex file\n");
			TRACE("\tUsage: \\readhex\n");
		}
		else if (strcmp((const char *)CommandName, "burnhex") == 0)
		{
			TRACE("\tCommand: burnhex\n");
			TRACE("\tFunction: Erase & burn flash hex file\n");
			TRACE("\tUsage: \\burnhex <The combination value of partition IDs, value from 0x1 to 0x7>\n");
		}
		else if (strcmp((const char *)CommandName, "ocmversion") == 0)
		{
			TRACE("\tCommand: ocmversion\n");
			TRACE("\tFunction: read OCM version from register\n");
			TRACE("\tUsage: \\ocmversion\n");
		}
		else if (strcmp((const char *)CommandName, "readintr") == 0)
		{
...
static void burnhex(void){
	uint32_t partition_ids = 0;
	uint8_t id;
	int total_num = PARTITION_ID_MAX;

	if (sscanf((const char *)g_CmdLineBuf, "\\%*s %x", &partition_ids) == 1)
	{
		if ((partition_ids == 0) || (partition_ids > 0x7)) {
			goto invalid_para;
		}

		power_restart();
		for (id = 0; id < total_num; id++) {
			if (partition_ids & (0x1 << id)) {
				command_erase_partition(id);
			}
		}
		burn_hex_prepare();
		delay_ms(1000);
...
invalid_para:
	TRACE("\n\tBad parameter! Usage:\n");
	TRACE("\t\\burnhex <The combination value of partition IDs>\n");
	TRACE("Which partition do you want to update?\n");
	TRACE("bit 0: Main OCM FW        --> 0x1\n");
	TRACE("bit 1: Secure OCM FW      --> 0x2\n");
	TRACE("bit 2: HDCP 1.4 & 2.2 key --> 0x4\n");
	TRACE("Example: value '1' for Main OCM FW only\n");
	TRACE("Example: value '3' for Main OCM FW + Secure OCM FW\n");
	TRACE("Example: value '7' for Main OCM FW + Secure OCM FW + HDCP 1.4 & 2.2 key\n\n");
}
